...
//show the content
	...
	echo "	<tr>\n";
	echo "		<td width='50%' align='left' nowrap='nowrap'><b>Call Block</b></td>\n";
	echo "		<td width='50%' align='right'>&nbsp;</td>\n";
	echo "	</tr>\n";
	echo "	<tr>\n";
	echo "		<td align='left' colspan='2'>\n";
	echo "			A list of numbers from which to block calls.<br /><br />\n";
	echo "		</td>\n";
	echo "	</tr>\n";
	echo "</table>\n";
	//echo "$domain_uuid\n";
	...
	echo "<tr>\n";
	echo th_order_by('blocked_caller_number', 'Number', $order_by, $order);
	echo th_order_by('blocked_caller_name', 'Name', $order_by, $order);
	echo th_order_by('blocked_call_count', 'Count', $order_by, $order);
	echo th_order_by('date_added', 'Date Added', $order_by, $order);
	echo th_order_by('blocked_call_action', 'Action', $order_by, $order);
	echo th_order_by('block_call_enabled', 'Enabled', $order_by, $order);
	echo "<td align='right' width='42'>\n";
	if (permission_exists('callblock_add')) {
		echo "	<a href='callblock_edit.php' alt='add'>$v_link_label_add</a>\n";
	}
	echo "</td>\n";
	echo "<tr>\n";
	...
	if ($result_count > 0) {
		foreach($result as $row) {
			...
			if (permission_exists('callblock_edit')) {
				echo "		<a href='callblock_edit.php?id=".$row['blocked_caller_uuid']."' alt='edit'>$v_link_label_edit</a>\n";
			}
			if (permission_exists('callblock_delete')) {
				echo "		<a href='callblock_delete.php?id=".$row['blocked_caller_uuid']."' alt='delete' onclick=\"return confirm('Do you really want to delete this?')\">$v_link_label_delete</a>\n";
			};
			echo "</tr>\n";
			if ($c==0) { $c=1; } else { $c=0; }
		} //end foreach
