...
if (count($_POST)>0 && strlen($_POST["persistformvar"]) == 0) {
	...
	//set the default extension_continue to false
	...
	//v_dialplan_details
		if ($action == "update" && permission_exists('fifo_edit')) {
			...
			if (permission_exists('fifo_add')) {
				echo "	<a href='v_fifo_details_edit.php?id2=".$dialplan_include_uuid."' alt='add'>$v_link_label_add</a>\n";
			}
			echo "</td>\n";
			echo "<tr>\n";
			...
			if ($resultcount == 0) {
				...
				foreach($result as $row) {
					...
					if (permission_exists('fifo_edit')) {
						echo "		<a href='v_fifo_details_edit.php?id=".$row[dialplan_includes_detail_uuid]."&id2=".$dialplan_include_uuid."' alt='edit'>$v_link_label_edit</a>\n";
					}
					if (permission_exists('fifo_delete')) {
						echo "		<a href='v_fifo_details_delete.php?id=".$row[dialplan_includes_detail_uuid]."&id2=".$dialplan_include_uuid."' alt='delete' onclick=\"return confirm('Do you really want to delete this?')\">$v_link_label_delete</a>\n";
					}
					echo "	</td>\n";
					echo "</tr>\n";
					if ($c==0) { $c=1; } else { $c=0; }
