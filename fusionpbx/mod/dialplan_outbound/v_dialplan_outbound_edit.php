...
//show the content
	...
	echo "		<input type='button' class='btn' name='' alt='copy' onclick=\"if (confirm('Do you really want to copy this?')){window.location='v_dialplan_outbound_copy.php?id=".$dialplan_include_uuid."';}\" value='Copy'>\n";
	...
	if ($action == "update" && permission_exists('outbound_route_edit')) {
		...
		//show conditions
			...
			if (permission_exists('outbound_route_add')) {
				echo "	<a href='v_dialplan_outbound_details_edit.php?id2=".$dialplan_include_uuid."' alt='add'>$v_link_label_add</a>\n";
			}
			echo "</td>\n";
			echo "<tr>\n";
			...
			if ($resultcount == 0) {
				...
				foreach($result as $row) {
					...
					if (permission_exists('outbound_route_edit')) {
						echo "		<a href='v_dialplan_outbound_details_edit.php?id=".$row[dialplan_includes_detail_uuid]."&id2=".$dialplan_include_uuid."' alt='edit'>$v_link_label_edit</a>\n";
					}
					if (permission_exists('outbound_route_delete')) {
						echo "		<a href='v_dialplan_outbound_details_delete.php?id=".$row[dialplan_includes_detail_uuid]."&id2=".$dialplan_include_uuid."' alt='delete' onclick=\"return confirm('Do you really want to delete this?')\">$v_link_label_delete</a>\n";
					}
					echo "	</td>\n";
					echo "</tr>\n";
					if ($c==0) { $c=1; } else { $c=0; }
