	...
	if ($resultcount == 0) {
		...
		foreach($result as $row) {
			...
			if (permission_exists('call_forward_edit')) {
				echo "		<a href='v_call_forward_edit.php?id=".$row[dialplan_include_uuid]."' alt='edit'>$v_link_label_edit</a>\n";
			}
			if (permission_exists('call_forward_delete')) {
				echo "		<a href='v_call_forward_delete.php?id=".$row[dialplan_include_uuid]."' alt='delete' onclick=\"return confirm('Do you really want to delete this?')\">$v_link_label_delete</a>\n";
			}
			echo "   </td>\n";
			echo "</tr>\n";
			if ($c==0) { $c=1; } else { $c=0; }
