...
//show the content
	...
	echo "<tr>\n";
	echo thorder_by('adr_type', 'Type', $order_by, $order);
	//echo thorder_by('adr_street', 'Street Address', $order_by, $order);
	//echo thorder_by('adr_extended', 'Extended Address', $order_by, $order);
	echo thorder_by('adr_locality', 'City', $order_by, $order);
	echo thorder_by('adr_region', 'Region', $order_by, $order);
	//echo thorder_by('adr_postal_code', 'Postal Code', $order_by, $order);
	echo thorder_by('adr_country', 'Country', $order_by, $order);
	//echo thorder_by('adr_latitude', 'Latitude', $order_by, $order);
	//echo thorder_by('adr_longitude', 'Longitude', $order_by, $order);
	echo "<th>Tools</th>\n";
	...
	if ($result_count > 0) {
		foreach($result as $row) {
			...
			echo "		<a href='v_contacts_adr_edit.php?contact_uuid=".$row['contact_uuid']."&id=".$row['contact_adr_uuid']."' alt='edit'>$v_link_label_edit</a>\n";
			echo "		<a href='v_contacts_adr_delete.php?contact_uuid=".$row['contact_uuid']."&id=".$row['contact_adr_uuid']."' alt='delete' onclick=\"return confirm('Do you really want to delete this?')\">$v_link_label_delete</a>\n";
			echo "	</td>\n";
			echo "</tr>\n";
			if ($c==0) { $c=1; } else { $c=0; }
		} //end foreach
