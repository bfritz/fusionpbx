...
//to do list
	//make it possible to update mysql and postgresql schema
	//get the latest php source
	//option to update the menu
...
*/
if ($display_results) {
	echo "<strong>Database Type: ".$dbtype. "</strong><br /><br />";
}
...
//list all tables
	...
	foreach ($result as &$row) {
		...
		//check if the table exists
			if (db_table_exists($db, $dbtype, $dbname, $table_name)) {
				if ($display_results) {
				...
				if (preg_match("/\(.*\)/", $sql, $matches)) {
					...
					//show the list of columns
						...
						foreach ($column_array as &$column_row) {
							...
							if (db_column_exists ($db, $dbtype, $dbname, $table_name, $column_row['column_name'])) {
								if ($display_results) {
